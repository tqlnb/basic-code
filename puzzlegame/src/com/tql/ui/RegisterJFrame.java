...
public class RegisterJFrame extends JFrame implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if(e.getSource() == submit){
            ...
            if(!username.getText().matches("[a-zA-Z0-9]{4,16}")){
                showDialog("用户名应由4-16位大小写字母或者数字组成");
                return;
            }
            if(!password.getText().matches("\\S*(?=\\S{6,})(?=\\S*\\d)(?=\\S*[A-Z])(?=\\S*[a-z])\\S*")){
                showDialog("密码至少6位,并同时包含大小写字母和数字");
                return;
            }
            //4.判断用户名是否已经重复
            ...
            //6.写入文件
            FileUtil.writeLines(allUsers,"C:\\Users\\tql\\IdeaProjects\\basic-code\\puzzlegame\\userinfo.txt", StandardCharsets.UTF_8);
            //7.提示注册成功
            showDialog("注册成功");
            //关闭注册页面,打开登陆界面
    ...
    public boolean containsUsername(String usernameText){
        for (User u : allUsers) {
            if(u.getUsername().equals(usernameText)){
                return true;
            }
        }
        return false;
